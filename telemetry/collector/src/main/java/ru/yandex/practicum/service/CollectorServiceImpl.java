...
public class CollectorServiceImpl implements CollectorService {
    ...
    @Value("${app.topics.hubs}") private String hubTopic;
    @Value("${app.topics.sensors}") private String sensorTopic;

    private final AvroSerializer avroSerializer;

    @Override
    public void sendHubEvent(HubEvent hubEvent) {
        SpecificRecordBase record = HubEventMapper.toAvro(hubEvent);
    ...
    public void sendSensorEvent(SensorEvent sensorEvent) {
        SpecificRecordBase record = SensorEventMapper.toAvro(sensorEvent);
